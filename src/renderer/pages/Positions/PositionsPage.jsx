...
function PositionsPage() {
  ...
  const loadPositions = useCallback(async () => {
    setLoading(true);
    try {
      const data = await window.electronAPI.positions.getAll();
      setPositions(data);
    } catch (e) {
  ...
  const openAdd = () => {
    setForm({ code: "", name: "", description: "", sort_order: positions.length + 1, is_active: 1 });
    setFormErrors({});
    setDialogItem(null);
    setDialogOpen(true);
  };
  ...
  const validateForm = () => {
    const errs = {};
    if (!form.code.trim()) errs.code = "Mã chức vụ là bắt buộc";
    if (!form.name.trim()) errs.name = "Tên chức vụ là bắt buộc";
    setFormErrors(errs);
  ...
  const handleSave = async () => {
    ...
    let result;
    if (dialogItem) {
      result = await window.electronAPI.positions.update(dialogItem.id, payload);
    } else {
      result = await window.electronAPI.positions.create(payload);
    }
    if (result.ok) {
      showSnackbar(dialogItem ? "Cập nhật chức vụ thành công" : "Thêm chức vụ thành công");
      setDialogOpen(false);
      loadPositions();
  ...
  const handleDelete = async () => {
    const result = await window.electronAPI.positions.delete(deleteDialog.id);
    if (result.ok) {
      showSnackbar("Đã xóa chức vụ");
  ...
  const columns = [
    ...
    { field: "name", headerName: "Tên chức vụ", flex: 1, minWidth: 160 },
    ...
    <Box sx={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflow: "hidden" }}>
              ...
              <DataGridToolbarActions
                ...
                rightActions={
                  <>
                    ...
                    <StandardButton primary={false} icon={<RefreshIcon />} label="Làm mới" onClick={() => { loadPositions(); showSnackbar("Dữ liệu đã được làm mới"); }} />
                  </>
                }
              />
            ),
        ...
        {drawerItem && (
          <>
            <Box sx={{ p: 3, pb: 2, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #e2e8f0" }}>
              <Typography variant="h6" sx={{ fontWeight: 700 }}>Chi tiết chức vụ</Typography>
            ...
            <Box sx={{ flexGrow: 1, overflowY: "auto", p: 3 }}>
              <Box sx={{ mb: 3, p: 2, bgcolor: "#f8fafc", borderRadius: 2, textAlign: "center" }}>
                <Chip label={drawerItem.code} color="secondary" sx={{ fontWeight: 700, fontSize: 14, mb: 1 }} />
                <Typography variant="h6" sx={{ fontWeight: 700 }}>{drawerItem.name}</Typography>
              </Box>
              <InfoRow label="Mã" value={drawerItem.code} />
              <Divider />
              <InfoRow label="Tên chức vụ" value={drawerItem.name} />
              <Divider />
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
        <DialogTitle sx={{ m: 0, p: 2.5, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e2e8f0" }}>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>{dialogItem ? "Cập nhật chức vụ" : "Thêm chức vụ mới"}</Typography>
        ...
        <DialogContent sx={{ p: 3, display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
          <TextField
            label="Mã chức vụ *" value={form.code}
          ...
          />
          <TextField
            label="Tên chức vụ *" value={form.name}
        ...
        <DialogActions sx={{ p: 2.5, borderTop: "1px solid #e2e8f0", bgcolor: "#f8fafc" }}>
          ...
          <Button onClick={handleSave} variant="contained" color="primary">
            {dialogItem ? "Lưu thay đổi" : "Thêm chức vụ"}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!deleteDialog} onClose={() => setDeleteDialog(null)}>
        ...
        <DialogContent>
          <Typography>Bạn có chắc muốn xóa chức vụ <b>{deleteDialog?.name}</b> không?</Typography>
        </DialogContent>
        <DialogActions>
